...
const AdminWorkshop = () => {
  ...
  const removeItem = (id: string) => {
    setUsageItems((prev) => prev.filter((i) => i.id !== id));
  };

  const finalizeUsage = async () => {
    if (usageItems.length === 0) {
      toast.error("Agrega productos consumidos");
      return;
    }
    setProcessing(true);
    try {
      const items = usageItems.map((i) => ({
        id: i.id,
        name: i.name,
      ...
      toast.success("¡Consumo de taller registrado y stock actualizado! 🔧");
      setUsageItems([]);
      const { data } = await supabase.from("products").select("*").order("name");
      setProducts((data as Product[]) || []);
    } catch (err: any) {
          ...
          <div className="border border-border rounded-lg divide-y divide-border max-h-96 overflow-y-auto">
            {filtered.map((p) => (
              <button
                ...
                onClick={() => addItem(p)}
                disabled={p.stock <= 0}
                className="w-full text-left p-3 hover:bg-muted/50 transition-colors flex items-center justify-between disabled:opacity-50"
              >
                <div>
                  <p className="font-medium text-sm">{p.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {p.code} {p.specifications ? `· ${p.specifications}` : ""}
                  </p>
                </div>
      ...
      <div className="space-y-4">
        ...
        {usageItems.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8 border border-dashed border-border rounded-lg">
            Busca y agrega productos consumidos en el taller
          </p>
        ) : (
            ...
            {usageItems.map((item) => (
              <div key={item.id} className="border border-border rounded-lg p-3 bg-card">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => updateQty(item.id, -1)} className="p-1 rounded bg-muted hover:bg-muted/80">
                    <Minus size={14} />
                  </button>
        ...
        {usageItems.length > 0 && (
          <div className="border-t border-border pt-4">
            <Button onClick={finalizeUsage} disabled={processing} className="w-full gap-2" size="lg">
              {processing ? <Loader2 size={18} className="animate-spin" /> : <Wrench size={18} />}
              {processing ? "Procesando..." : "Registrar Consumo de Taller"}
            </Button>
          </div>
        )}
      </div>
