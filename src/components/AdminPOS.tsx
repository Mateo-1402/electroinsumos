...
const AdminPOS = () => {
  ...
  const addToSale = (product: Product) => {
    ...
    setSaleItems((prev) => {
      ...
      if (existing) {
        ...
        return prev.map((i) =>
          i.id === product.id
            ? { ...i, quantity: i.quantity + 1, price: i.unitPrice * (i.quantity + 1) }
            : i
        );
      }
      return [
  ...
  const updateItemPrice = (id: string, newPrice: string) => {
    ...
    setSaleItems((prev) =>
      prev.map((i) => (i.id === id ? { ...i, price, unitPrice: price / i.quantity } : i))
    );
  };
  ...
  const finalizeSale = async () => {
    ...
    try {
      ...
      const { error } = await supabase.rpc("create_pos_sale", {
        p_customer_name: customerName || "Mostrador",
        p_items: items,
        p_final_price: total,
      });
      ...
      <div className="space-y-4">
              ...
              <div key={item.id} className="border border-border rounded-lg p-3 bg-card">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1">
                    <button onClick={() => updateItemQuantity(item.id, -1)} className="p-1 rounded bg-muted hover:bg-muted/80">
                      <Minus size={14} />
                    </button>
        ...
        {saleItems.length > 0 && (
          <div className="border-t border-border pt-4 space-y-3">
            ...
            <Button onClick={finalizeSale} disabled={processing} className="w-full gap-2" size="lg">
